...
class Report {
	...
	void PrintCert(TextWriter w, X509Chain xchain, int num)
	{
		...
		if (xc == null) {
			...
			if (xc.HashAlgorithm == "SHA-256")
			{
			Console.WriteLine("Checked for SHA successfully!:- The server is signed by robust sign-hash algorithm SHA-256.");
			Console.WriteLine("Refrain from using SHA 1 strictly. For more details on this vulnerability, please check following link:");
			Console.WriteLine("https://security.googleblog.com/2014/09/gradually-sunsetting-sha-1.html");

			}
			
			else if (xc.HashAlgorithm == "SHA-1"){ 
			Console.WriteLine("*****ALERT*****: Not a powerful Cipher!!!!!!!!!!!!!!!!!!!!!!");
			      }
